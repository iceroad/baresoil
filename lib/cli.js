...
function main(args) {
  ...
  if (cmd.requiresProject) {
    ...
    try {
      ...
      console.error([
        'Error: ' + colutil.error(
          'This command must be run inside a Baresoil project directory.'),
        '',
        colutil.help(
  ...
  if (cmd.requiresApp && !appToken) {
    console.error([
      'Error: ' + colutil.error('No deployment key found for this project.'),
      '',
      fmt('Ensure that you have registered a subdomain for your project using %s.',
