...
function main(args) {
  ...
  if (cmd.requiresApp && !appToken) {
    console.error([
      'Error: No deployment key found for this project.'.red,
      '',
      'Ensure that you have:',
      '',
      fmt(
          '  * logged in to your Baresoil Cloud account using %s',
          'baresoil login'.yellow),
      fmt(
          '  * have registered a subdomain for your project using %s',
          'baresoil register'.yellow),
      '',
      fmt(
          'These steps should create a file called %s in your ' +
          'project\'s root directory.', path.basename(appTokenPath).yellow),
      fmt(
          'This file contains credentials that allow you to use %s.',
          ('baresoil ' + cmdName).yellow),
      '',
      'Looked in: ' + appTokenPath.red,
    ].join('\n'));
    return process.exit(1);
  }
