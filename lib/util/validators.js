...
module.exports = {
  ...
  baresoil_subdomain: function(v) {
    ...
    var domain = v;
    var matchingTld = _.find(_.concat(domains, ['baresoil.io']), function(tld) {
      var suffix = domain.substr(domain.length - tld.length);
      return suffix === tld;
    });
    if (!matchingTld) {
      return fmt(
    ...
    var prefix = domain.substr(0, domain.length - matchingTld.length - 1);
